...
public class SungJukServiceV3Impl implements SungJukServiceV3 {
    ...
    public void initSungJuk() {
        ...
        try {
            ...
            ex.printStackTrace();
            System.out.println("파일이 없거나 시스템오류 입니다!@!@");
        } finally {
            if (ois != null)
                try {
                    ois.close();
...
// try-with-resources  구문 사용
        try (
            ...
            // json파일의 모든내용을 한번에 읽어 lines 변수에 저장
    ...
    public void saveSungJuk3() {
        ...
        try (
            ...
            String result = "";
            bw.write("{\"sj\":[");    // json 배열시작
            for (SungJukVO sj : sjdata) {
                result = String.format(jsonfmt, sj.getName(),
                        sj.getKor(), sj.getEng(), sj.getMat(),
                        sj.getTot(), sj.getAvg(), sj.getGrd());
                // {'name':'혜교','kor':55,'eng':66,'mat':77,
                // 'tot':198,'avg':66,'grd':'양'},
                bw.write(result);
            }
            bw.write("]}");     // json 배열 끝
        } catch (Exception ex) {
