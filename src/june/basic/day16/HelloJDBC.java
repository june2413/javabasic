...
public class HelloJDBC {
    public static void main(String[] args) {
        ...
        // 작성해두면 어떤 DBMS라도 코드변경없이
        // 동일하게 작동할 수 있음

        // JDK에 포함된 JDBC인터페이스는 java.sql 패키지 아래에 있음
        // 하지만, 이것만으로는 작동하지 않고
        // 각 데이터베이스 제조사가 JDBC인터페이스를 구현해서
        // 작성한 클래스가 필요한데, 이것을 JDBC 드라이버 매니저라함
        // JDBC드라이버 매니저는 데이터베이스 제조사 홈페이지에서 내려받을 수 있음

        // amazon 데이터베이스 접속 확인
        String drv = "org.mariadb.jdbc.Driver";
        String amz = "bigdata.chrpnj0d47aq.ap-northeast-2.rds.amazonaws.com";
        ...
        // 2.데이터베이스 서버 접속
        try (
                Connection conn = DriverManager. getConnection(ourl,ousr,opwd);
        ){
            if (conn != null)
                System.out.println("데이터베이스 연결 성공!!");
        } catch (Exception ex){
        ...
        // 4. vmware에 구축된 postgre 서버 접속 테스트
        ...
        // 2.데이터베이스 서버 접속
        try (
                Connection conn = DriverManager. getConnection(purl,pusr,ppwd);
        ){
            if (conn != null)
                System.out.println("데이터베이스 연결 성공!!");
        } catch (Exception ex){
